...
class Game extends Component {
  ...
  checkThumbnailImageAndImage = id => {
    ...
    if (activeId === id) {
      ...
      const newActiveId = imagesList[randomNumber].id
      this.setState(prevState => ({
        score: prevState.score + 1,
        activeImage: randomImage,
  ...
  render() {
    const {activeTabId, activeImage, score, seconds, showResult} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        {showResult ? (
          ...
          <>
            <img src={activeImage} alt="" className="random-image" />
            <ul className="tab-container">
              {tabsList.map(each => (
                <TabItem
                  ...
                  activeTabIdClassname={activeTabId === each.tabId}
                />
              ))}
            </ul>
            <ul className="thumbnail-container">
              {filteredImagesList.map(each => (
                <ThumbnailImages
                  ...
                  key={each.id}
                  checkThumbnailImageAndImage={this.checkThumbnailImageAndImage}
                />
              ))}
            </ul>
          </>
